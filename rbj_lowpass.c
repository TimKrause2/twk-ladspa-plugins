	...
	unsigned long p_sample_count )
{
	...
	LADSPA_Data l_omega = 2.0*M_PI* *l_pLowpass->m_pport[PORT_FREQUENCY] /
	l_pLowpass->m_sample_rate;
	LADSPA_Data l_sin_omega = sinf( l_omega );
	LADSPA_Data l_cos_omega = cosf( l_omega );
	LADSPA_Data l_alpha = l_sin_omega /( 2 * *l_pLowpass->m_pport[PORT_Q]);
	LADSPA_Data l_a0 = 1.0 + l_alpha;
	LADSPA_Data l_a1 = -2.0 * l_cos_omega / l_a0;
	LADSPA_Data l_a2 = (1.0 - l_alpha) / l_a0;
	LADSPA_Data l_b0 = (1.0 - l_cos_omega) / 2.0 / l_a0;
	LADSPA_Data l_b1 = (1.0 - l_cos_omega) / l_a0;
	LADSPA_Data l_b2 = (1.0 - l_cos_omega) / 2.0 / l_a0;
	LADSPA_Data l_G = exp10f( *l_pLowpass->m_pport[PORT_GAIN] / 20.0 );
	for(l_sample=0;l_sample<p_sample_count;l_sample++){
		LADSPA_Data l_m = *l_psrc - l_a1*l_pLowpass->m_z1 - l_a2*l_pLowpass->m_z2;
